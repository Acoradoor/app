    ...
    $.widget('ui.scrollTabs', $.ui.tabs, {
        ...
        options: {
            scrollOptions: {
                ...
                customMoveNextHTML: '<button class="stNavNextArrow ui-state-active" title="Posteriro">' +
                    '<span class="ui-icon ui-icon-caret-1-e">Next tab</span></button>',
                ...
                customMovePreviousHTML: '<button class="stNavPrevArrow ui-state-active" title="Anterior">' +
                    '<span class="ui-icon ui-icon-caret-1-w">Previous tab</span></button>',
                easing: 'swing',
                enableDebug: false,
        ...
        addTab1: function (ids, header, panelContent) {
            var newId = $({}).uniqueId()[0].id;
            var tab = $('<li><a href="#' + ids + '" role="tab">' + header + '</a></li>');
            var panel = this._createPanel(ids);
            panel.html(panelContent);
            this.$ul.append(tab);
            panel.attr('aria-live', 'polite');
            if (panel.length) {
